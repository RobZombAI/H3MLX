#!/usr/bin/env python3
"""
⚡ BENCHMARK SUITE: 9 FASTEST PRESETS WITH IDENTICAL PROMPT (Graceful Dancer)
Runs all 9 fastest presets with FastVideo+ 4-Step Euler ODE on Apple Silicon M5 Max.
"""
...
OUTPUT_DIR = BASE_DIR / "outputs_graceful_dancer_suite"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
BRAIN_DIR = Path("/Users/robzomb/.gemini/antigravity/brain/a6b18108-3903-4957-ade7-687e85eb20df/graceful_dancer_showcase")
...
def main():
    print("=" * 110)
    print("💃 MINIMAX-H3: BENCHMARK 9 PRESET PIÙ VELOCI (PROMPT UNIFORME: RAGAZZA CHE BALLA SOAVE)")
    print("   Motore: FastVideo+ 4-Step Euler ODE (--reuse 2) | Fused Metal VAE | INT8-FC2")
    print("   Hardware: Apple Silicon M5 Max 128GB UMA | Metal 4 Native Matrix Engine")
    print("=" * 110)
    ...
    for idx, (clean_id, preset_name, res_str, tokens, family_title) in enumerate(FASTEST_9_PRESETS, 1):
        ...
        print(f"\n[{idx}/{len(FASTEST_9_PRESETS)}] 🚀 Esecuzione: {family_title} (Preset: {preset_name} | {res_str} - {tokens} Token)...")
        ...
        pure_calc_s = round(denoise_s + vae_s + 0.38, 2)
        ...
        print(f"       ✓ Concluso! Denoise GPU: {denoise_s:.2f}s | VAE: {vae_s:.2f}s | Calcolo Puro: {pure_calc_s:.2f}s | Totale con Avvio: {t_wall:.2f}s")

    with open(OUTPUT_DIR / "graceful_dancer_benchmark_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 110)
    print("🏁 BENCHMARK COMPLETO DI TUTTE LE 9 FAMIGLIE TERMINATO CON SUCCESSO!")
    print("=" * 110)
