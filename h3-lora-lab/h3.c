...
h3_result *h3_generate(h3_ctx *ctx, const char *prompt,
    ...
    if (params->sol_attn || getenv("H3_SOL_ATTN")) {
        float thresh = params->sol_attn_threshold > 0.0f ? params->sol_attn_threshold : 10.0f;
        uint32_t bsize = params->sol_attn_block_size > 0 ? params->sol_attn_block_size : 32;
        h3_dit_enable_sol_attn(dit, 1, thresh, bsize);
    }
    if (params->sol_cache || getenv("H3_SOL_CACHE")) {
        float thresh = params->sol_cache_thresh > 0.0f ? params->sol_cache_thresh : 0.08f;
        h3_dit_enable_sol_cache(dit, 1, thresh);
    }
    ...
    if (params->sol_stats || getenv("H3_SOL_STATS") || getenv("H3_PROFILE")) {
        h3_sol_stats sstats = {0};
        if (h3_dit_get_sol_stats(dit, &sstats) && sstats.total_steps > 0) {
            fprintf(stderr,
                    "\n--- Sol-Engine Acceleration Report ---\n"
                    "  Total Steps:      %llu\n"
                    "  Cached Steps:     %llu (%.1f%% skipped)\n"
                    "  Attention Calls:  %llu\n"
                    "  Sol-Attn Calls:   %llu (%.1f%% dynamic sparse)\n"
                    "  Forward Time:     %.3f s\n"
                    "--------------------------------------\n",
                    (unsigned long long)sstats.total_steps,
                    (unsigned long long)sstats.cached_steps,
                    sstats.total_steps ? (100.0 * (double)sstats.cached_steps / (double)sstats.total_steps) : 0.0,
                    (unsigned long long)sstats.total_attention_calls,
                    (unsigned long long)sstats.sol_attention_calls,
                    sstats.total_attention_calls ? (100.0 * (double)sstats.sol_attention_calls / (double)sstats.total_attention_calls) : 0.0,
                    sstats.total_forward_seconds);
        }
    }
    ...
    float crisp_gamma = (crisp_env && *crisp_env) ? (float)atof(crisp_env) : 0.0f;
    if (crisp_gamma > 0.001f && (size_t)temporal.video_t > 2) {
        size_t hw = (size_t)latent_h * (size_t)latent_w;
        size_t total_elements = (size_t)16 * (size_t)temporal.video_t * hw;
        float *filtered = malloc(total_elements * sizeof(float));
        if (filtered) {
            memcpy(filtered, video, total_elements * sizeof(float));
            for (size_t c = 0; c < 16; c++) {
                size_t c_offset = c * (size_t)temporal.video_t * hw;
                /* Frame 0: forward second-order difference */
                const float *f0 = video + c_offset;
                const float *f1 = video + c_offset + hw;
                const float *f2 = video + c_offset + 2 * hw;
                float *out0 = filtered + c_offset;
                ...
                for (size_t t = 1; t + 1 < (size_t)temporal.video_t; t++) {
                    const float *prev = video + c_offset + (t - 1) * hw;
                    const float *curr = video + c_offset + t * hw;
                    const float *next = video + c_offset + (t + 1) * hw;
                    float *out = filtered + c_offset + t * hw;
                    for (size_t i = 0; i < hw; i++) {
                ...
                size_t last_t = (size_t)temporal.video_t - 1;
                const float *fn = video + c_offset + last_t * hw;
                const float *fn_1 = video + c_offset + (last_t - 1) * hw;
                const float *fn_2 = video + c_offset + (last_t - 2) * hw;
                float *outn = filtered + c_offset + last_t * hw;
                for (size_t i = 0; i < hw; i++) {
    ...
    if (params->output_path && *params->output_path) {
        if (waveform.pcm && waveform.samples > 0) {
            ...
            if (!h3_ffmpeg_write_rgb24(
                    params->output_path, rgb8, frames.frames, output_width,
                    output_height, H3_FPS, detail, sizeof(detail))) {
                h3_set_error(ctx, "%s", detail);
                goto cleanup;
            }
        }
        if (params->master_10bit) {
            char master_path[1024];
            const char *dot = strrchr(params->output_path, '.');
            if (dot) {
                size_t prefix_len = (size_t)(dot - params->output_path);
                snprintf(master_path, sizeof(master_path), "%.*s_master_10bit%s",
                         (int)prefix_len, params->output_path, dot);
            } else {
                snprintf(master_path, sizeof(master_path), "%s_master_10bit.mp4",
                         params->output_path);
            }
            char filter[2048];
            if (params->fluid_60fps) {
                snprintf(filter, sizeof(filter),
                    "[0:v]minterpolate=fps=60:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1,"
                    "scale=%d:%d:flags=spline+accurate_rnd+full_chroma_int+full_chroma_inp,"
                    "cas=0.25,format=yuv420p10le[v]",
                    params->detailer_2k ? 2048 : 1920,
                    params->detailer_2k ? 1152 : 1080);
            } else {
                snprintf(filter, sizeof(filter),
                    "[0:v]scale=%d:%d:flags=spline+accurate_rnd+full_chroma_int+full_chroma_inp,"
                    "cas=0.25,format=yuv420p10le[v]",
                    params->detailer_2k ? 2048 : 1920,
                    params->detailer_2k ? 1152 : 1080);
            }
            ...
            snprintf(cmd, sizeof(cmd),
                "ffmpeg -y -loglevel error -i \"%s\" -filter_complex \"%s\" -map \"[v]\" -map 0:a "
                "-c:v hevc_videotoolbox -profile:v main10 -pix_fmt p010le -b:v 120M -tag:v hvc1 "
                "-color_primaries bt709 -color_trc bt709 -colorspace bt709 "
                "-c:a aac -b:a 320k -ar 48000 -movflags +faststart \"%s\"",
                params->output_path, filter, master_path);
            int m_rc = system(cmd);
            if (m_rc == 0) {
                fprintf(stderr, "h3: wrote Cinema Pro 10-Bit %s%s: %s\n",
                        params->detailer_2k ? "2K " : "1080p ",
                        params->fluid_60fps ? "60fps" : "24fps",
                        master_path);
            }
        }
