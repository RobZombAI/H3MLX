...
kernel void h3_layer_norm_f32(device const float *input [[buffer(0)]],
                              ...
                              uint3 thread_position [[thread_position_in_threadgroup]],
                              uint3 threadgroup_size [[threads_per_threadgroup]],
                              threadgroup float *shared_storage [[threadgroup(0)]]) {
    uint row = group.y;
    uint tid = thread_position.x;
    uint threads = threadgroup_size.x;
    if (row >= args.rows) return;
    ...
    for (uint column = tid; column < args.width; column += threads) {
        float val = x[column];
        sum += val;
        sq_sum = fma(val, val, sq_sum);
    }
    shared_storage[tid] = sum;
    shared_storage[threads + tid] = sq_sum;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    for (uint s = threads / 2; s > 0; s >>= 1) {
        if (tid < s) {
            shared_storage[tid] += shared_storage[tid + s];
            shared_storage[threads + tid] += shared_storage[threads + tid + s];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    float mean = shared_storage[0] / float(args.width);
    float variance = shared_storage[threads] / float(args.width) - mean * mean;
    float inverse = rsqrt(variance + args.epsilon);
    for (uint column = tid; column < args.width; column += threads)
...
kernel void h3_linear_int8_nax_r128(
    ...
    for (ushort element = 0; element < accum.get_capacity(); element++) {
        ...
        uint column = column_start + (uint)index[0];
        if (row < args.rows) {
            float b = (args.has_bias && bias) ? (float)bias[column] : 0.0f;
            output[row * args.output_dim + column] =
                (bfloat)((float)accum[element] * input_scales[row] *
                         weight_scales[column] + b);
        }
    }
}
    ...
    for (ushort element = 0; element < accum.get_capacity(); element++) {
        ...
        if (row < args.rows) {
            ...
            output[row * output_dim + column] =
                (bfloat)((float)accum[element] *
                         local_input_scales[(uint)index[1]] *
                         local_weight_scales[(uint)index[0]] + b);
        }
    }
}
...
/* Sol-Attn: Sparsifying Online Attention in Metal
 * Implements dynamic block-sparse attention with proxy scoring,
 * threshold-based gating, and proxy-score error correction in online softmax. */
struct sol_attn_args {
    uint rows;
    uint heads;
    uint head_dim;
    float scale;
    float threshold;
    uint block_size;
    uint output_head_major;
};

kernel void h3_sol_attn_bf16(device const ushort *query [[buffer(0)]],
                             device const ushort *key [[buffer(1)]],
                             device const ushort *value [[buffer(2)]],
                             device ushort *output [[buffer(3)]],
                             constant sol_attn_args &args [[buffer(4)]],
                             uint2 gid [[thread_position_in_grid]]) {
    uint row = gid.x;
    uint head = gid.y;
    if (row >= args.rows || head >= args.heads) return;

    uint rows = args.rows;
    uint head_dim = args.head_dim;
    float scale = args.scale;
    float threshold = args.threshold;
    uint block_size = args.block_size > 0 ? args.block_size : 32;

    // Offset pointers for current head: head-major layout [heads, rows, head_dim]
    device const ushort *q_ptr = query + (size_t)head * rows * head_dim + (size_t)row * head_dim;
    device const ushort *k_head = key + (size_t)head * rows * head_dim;
    device const ushort *v_head = value + (size_t)head * rows * head_dim;

    // Local registers for Query vector (head_dim is typically 128)
    float q_local[128];
    for (uint d = 0; d < head_dim && d < 128; d++) {
        q_local[d] = h3_bf16_to_f32(q_ptr[d]);
    }

    uint num_blocks = (rows + block_size - 1) / block_size;

    // Step 1: Compute proxy scores for all blocks to find maximum and determine dynamic threshold
    float max_proxy = -1e30f;

    for (uint b = 0; b < num_blocks; b++) {
        uint block_start = b * block_size;
        uint current_bs = (block_start + block_size <= rows) ? block_size : (rows - block_start);
        
        // Proxy score using subsampled representative token in the block
        uint rep_idx = block_start + (current_bs / 2);
        device const ushort *k_rep = k_head + (size_t)rep_idx * head_dim;
        
        float score = 0.0f;
        for (uint d = 0; d < head_dim && d < 128; d += 2) {
            score = fma(q_local[d], h3_bf16_to_f32(k_rep[d]), score);
            if (d + 1 < head_dim && d + 1 < 128)
                score = fma(q_local[d+1], h3_bf16_to_f32(k_rep[d+1]), score);
        }
        score *= scale;
        if (score > max_proxy) max_proxy = score;
    }

    // Dynamic threshold: blocks with proxy score within threshold of max are computed accurately
    float cutoff = max_proxy - threshold;

    // Step 2: Online Softmax Accumulation with Proxy-Score Reuse Correction
    float m_prev = -1e30f;
    float l_sum = 0.0f;
    float out_accum[128];
    for (uint d = 0; d < head_dim && d < 128; d++) out_accum[d] = 0.0f;
    ...
    for (uint b = 0; b < num_blocks; b++) {
        ...
        float p_score = 0.0f;
        for (uint d = 0; d < head_dim && d < 128; d += 2) {
            p_score = fma(q_local[d], h3_bf16_to_f32(k_rep[d]), p_score);
            if (d + 1 < head_dim && d + 1 < 128)
                p_score = fma(q_local[d+1], h3_bf16_to_f32(k_rep[d+1]), p_score);
        }
        p_score *= scale;

        if (p_score >= cutoff || b == 0) {
            // Active block: compute exact attention for each key/value in block
            for (uint k_idx = block_start; k_idx < block_start + current_bs; k_idx++) {
                device const ushort *k_row = k_head + (size_t)k_idx * head_dim;
                device const ushort *v_row = v_head + (size_t)k_idx * head_dim;

                float qk = 0.0f;
                for (uint d = 0; d < head_dim && d < 128; d++) {
                    qk = fma(q_local[d], h3_bf16_to_f32(k_row[d]), qk);
                }
                qk *= scale;

                float m_curr = max(m_prev, qk);
                float exp_old = exp(m_prev - m_curr);
                float exp_new = exp(qk - m_curr);

                l_sum = l_sum * exp_old + exp_new;
                for (uint d = 0; d < head_dim && d < 128; d++) {
                    out_accum[d] = fma(out_accum[d], exp_old, exp_new * h3_bf16_to_f32(v_row[d]));
                }
                m_prev = m_curr;
            }
        } else {
            // Pruned block: Sol-Attn Proxy-Score Reuse correction
            // Preserves background attention mass and avoids color/contrast drift
            float m_curr = max(m_prev, p_score);
            float exp_old = exp(m_prev - m_curr);
            float exp_proxy = exp(p_score - m_curr);

            l_sum = l_sum * exp_old + float(current_bs) * exp_proxy;

            // Representative value vector for pruned block
            device const ushort *v_rep = v_head + (size_t)rep_idx * head_dim;
            for (uint d = 0; d < head_dim && d < 128; d++) {
                out_accum[d] = fma(out_accum[d], exp_old, float(current_bs) * exp_proxy * h3_bf16_to_f32(v_rep[d]));
            }
            m_prev = m_curr;
        }
    }
    ...
    device ushort *out_ptr;
    if (args.output_head_major) {
        out_ptr = output + (size_t)head * rows * head_dim + (size_t)row * head_dim;
    } else {
        out_ptr = output + (size_t)row * args.heads * head_dim + (size_t)head * head_dim;
    }

    for (uint d = 0; d < head_dim && d < 128; d++) {
        out_ptr[d] = h3_f32_to_bf16(out_accum[d] * inv_l);
    }
}

/* 3D Latent Upsample in Metal: Trilinear interpolation across [Channels, T, H, W] */
struct latent_upsample_3d_args {
    uint channels;
    uint in_t;
    uint in_h;
    uint in_w;
...
kernel void h3_latent_upsample_3d_f32(device const float *input [[buffer(0)]],
                                      device float *output [[buffer(1)]],
                                      constant latent_upsample_3d_args &args [[buffer(2)]],
                                      uint3 gid [[thread_position_in_grid]]) {
    uint ow = gid.x;
    uint oh = gid.y;
    uint c_ot = gid.z;
    if (ow >= args.out_w || oh >= args.out_h) return;

    uint ot = c_ot % args.out_t;
    uint c = c_ot / args.out_t;
    if (c >= args.channels) return;

    // Coordinate mapping with center alignment
    float t_ratio = args.out_t > 1 ? float(args.in_t - 1) / float(args.out_t - 1) : 0.0f;
    float h_ratio = args.out_h > 1 ? float(args.in_h - 1) / float(args.out_h - 1) : 0.0f;
    float w_ratio = args.out_w > 1 ? float(args.in_w - 1) / float(args.out_w - 1) : 0.0f;

    float t_in = float(ot) * t_ratio;
    float h_in = float(oh) * h_ratio;
    float w_in = float(ow) * w_ratio;

    uint t0 = uint(floor(t_in));
    uint t1 = min(t0 + 1, args.in_t - 1);
    float dt = t_in - float(t0);

    uint h0 = uint(floor(h_in));
    uint h1 = min(h0 + 1, args.in_h - 1);
    float dh = h_in - float(h0);

    uint w0 = uint(floor(w_in));
    uint w1 = min(w0 + 1, args.in_w - 1);
    float dw = w_in - float(w0);

    #define IN_INDEX(chan, t_idx, h_idx, w_idx) \
        (((size_t)(chan) * args.in_t * args.in_h * args.in_w) + \
         ((size_t)(t_idx) * args.in_h * args.in_w) + \
         ((size_t)(h_idx) * args.in_w) + (w_idx))

    float c000 = input[IN_INDEX(c, t0, h0, w0)];
    float c001 = input[IN_INDEX(c, t0, h0, w1)];
    float c010 = input[IN_INDEX(c, t0, h1, w0)];
    float c011 = input[IN_INDEX(c, t0, h1, w1)];
    float c100 = input[IN_INDEX(c, t1, h0, w0)];
    float c101 = input[IN_INDEX(c, t1, h0, w1)];
    float c110 = input[IN_INDEX(c, t1, h1, w0)];
    float c111 = input[IN_INDEX(c, t1, h1, w1)];

    #undef IN_INDEX

    // Interpolate along W
    float c00 = mix(c000, c001, dw);
    float c01 = mix(c010, c011, dw);
    float c10 = mix(c100, c101, dw);
    float c11 = mix(c110, c111, dw);

    // Interpolate along H
    float c0 = mix(c00, c01, dh);
    float c1 = mix(c10, c11, dh);

    // Interpolate along T
    float result = mix(c0, c1, dt);

    size_t out_idx = ((size_t)c * args.out_t * args.out_h * args.out_w) +
                     ((size_t)ot * args.out_h * args.out_w) +
                     ((size_t)oh * args.out_w) + ow;
    output[out_idx] = result;
}

struct vae_speculative_tile_bypass_args {
    uint tile_w;
    uint tile_h;
    uint tile_t;
    uint channels;
...
kernel void h3_speculative_fused_vae_tile_bypass_f32(
    device const float *prev_decoded_rgb [[buffer(0)]],
    device float *out_tile_rgb [[buffer(1)]],
    constant vae_speculative_tile_bypass_args &args [[buffer(2)]],
    uint3 gid [[thread_position_in_grid]])
{
    uint w = gid.x;
    uint h = gid.y;
    uint t = gid.z;
    if (w >= args.tile_w || h >= args.tile_h || t >= args.tile_t) return;

    size_t spatial_idx = (size_t)t * args.tile_h * args.tile_w + (size_t)h * args.tile_w + w;
    size_t plane_stride = (size_t)args.tile_t * args.tile_h * args.tile_w;

    out_tile_rgb[spatial_idx + 0 * plane_stride] = prev_decoded_rgb[spatial_idx + 0 * plane_stride];
    out_tile_rgb[spatial_idx + 1 * plane_stride] = prev_decoded_rgb[spatial_idx + 1 * plane_stride];
    out_tile_rgb[spatial_idx + 2 * plane_stride] = prev_decoded_rgb[spatial_idx + 2 * plane_stride];
}
