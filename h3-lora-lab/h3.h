...
typedef struct {
    ...
    int preview_denoise;
    /* Sol-Engine / Sol-Attn Acceleration Options */
    int sol_attn;
    float sol_attn_threshold;
    uint32_t sol_attn_block_size;
    int sol_cache;
    float sol_cache_thresh;
    int sol_draft_refine;
    int sol_draft_steps;
    int sol_stats;
    /* Speech / Dialogue audio overlay track (mixed natively in C) */
    const char *speech_audio;
    /* Output in Apple VideoToolbox 10-Bit Main10 HEVC */
    int master_10bit;
    /* 2K Latent Detailer & Lanczos-CAS texture refinement */
    int detailer_2k;
