...
/* Sol-Engine / Sol-Attn Acceleration Statistics */
typedef struct {
    uint64_t total_attention_calls;
    uint64_t sol_attention_calls;
    uint64_t total_steps;
    uint64_t cached_steps;
    double total_forward_seconds;
    float effective_sparsity;
} h3_sol_stats;
...
/* Sol-Engine Multi-Stage Draft & Refine Sampler */
...
/* 3D Latent Upsampling helper (CPU / GPU fallback) */
