...
def synthesize_gunshot(duration: float = 0.5):
    """Synthesizes a tactical gunshot (transient crack + sub-bass 60Hz + metallic resonance)."""
    import random
    samples = int(duration * SAMPLE_RATE)
    data = [0.0] * samples
    for i in range(samples):
        t = i / SAMPLE_RATE
        # Transient crack (exponential decay white noise)
        crack = (random.random() * 2.0 - 1.0) * math.exp(-t * 60.0) * 1.5
        # Sub-bass thud (65 Hz decaying)
...
def synthesize_sword_clash(duration: float = 0.8):
    """Synthesizes steel blade / katana clash impact and harmonic ring."""
    import random
    samples = int(duration * SAMPLE_RATE)
    data = [0.0] * samples
    for i in range(samples):
        ...
        # Tempered steel harmonics (1200Hz, 2850Hz, 4900Hz)
...
def synthesize_body_slam(duration: float = 0.7):
    """Synthesizes a close-quarters body slam impact."""
    import random
    samples = int(duration * SAMPLE_RATE)
    data = [0.0] * samples
    for i in range(samples):
...
def synthesize_rain_ambience(duration: float):
    """Synthesizes continuous rain ambience on asphalt."""
    import random
    samples = int(duration * SAMPLE_RATE)
    data = [0.0] * samples
    last = 0.0
...
def create_foley_track(prompt: str, duration: float, out_wav: Path):
    """Analyzes prompt keywords and synthesizes a tailored 48 kHz stereo Foley soundscape."""
...
def inject_foley_to_video(video_in: Path, prompt: str, video_out: Path):
    """Synthesizes Foley audio track and muxes it into MP4 at 48 kHz."""
    probe_cmd = [
...
if __name__ == "__main__":
    ...
    print(f"Generating Foley sound design for: {v_in.name}")
    if inject_foley_to_video(v_in, p, v_out):
        print(f"Video with Foley sound saved to: {v_out}")
    else:
        print("Error during audio injection.", file=sys.stderr)
